...
export const PhotoService = {
  getImages: function() {
    ...
    const existingArray = [
      ...
      { title: 'Campeonato de atletismo', alt: 'En la final del campeonato de atletismo, organizado por el IPD, felicitamos a nuestros alumnos por su destacada participación, logrando muchos de ellos un podio.'},
      { title: 'Día del estudiante', alt: 'Compartir y actuación por el día del estudiante, nuestros niños y adolescentes se divirtieron mucho y nosotros nos sentimos felices por ello.'},
      { title: 'Ensamble musical CORPAIDINO', alt: 'En CORPAIDOS estamos convencidos que el desarrollo artistico musical favorece el desarrollo intelectual y social.'}
    ];
    ...
    const formattedImages = images.map((src, index) => {
      ...
      const existingObject = existingArray.find(obj => obj.title === fileName);
